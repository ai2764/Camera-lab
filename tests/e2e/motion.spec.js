...
test("Motion tab exposes Text to Motion, SCAIL2, and 3D Motion sub tabs", async ({ page }) => {
  ...
  await expect(page.locator("#motionScailPreviewRow #motionGuide")).toBeVisible();
  await expect(page.locator("#motionScailPreviewRow #motionResult")).toBeVisible();
  ...
  await expect(page.locator("#motion3dPanel .video-card")).toBeVisible();
  ...
  await expect(page.locator("#motion3dPanel .app-page > .stage-results")).toBeVisible();
  await expect(page.locator("#motion3dPanel .app-page > .stage-results")).toContainText("Generated videos");
  await expect(page.locator("#motion3dPanel .stage-panel > .stage-results")).toHaveCount(0);
  await expect(page.locator("#motion3dPanel .control-panel > .stage-results")).toHaveCount(0);
  ...
  const motion3dShellBox = await page.locator("#motion3dPanel .app-shell").boundingBox();
  ...
  const motion3dVideoBox = await page.locator("#motion3dPanel .video-card").boundingBox();
  expect(motion3dShellBox.y + motion3dShellBox.height).toBeLessThanOrEqual(page.viewportSize().height);
  ...
  expect(motion3dVideoBox.y + motion3dVideoBox.height).toBeLessThanOrEqual(page.viewportSize().height);
...
test("Motion tab generates guide before rendering final result", async ({ page }) => {
  ...
  await expect(page.locator("#motionTrimStart")).toHaveValue("3.5");
  await expect(page.locator("#motionTrimEnd")).toHaveValue("4");
  await page.evaluate(() => {
    document.querySelector("#motionGuide").currentTime = 1;
  });
  await page.locator("#motionTrimSetEnd").click();
  await expect(page.locator("#motionTrimStart")).toHaveValue("0");
  await expect(page.locator("#motionTrimEnd")).toHaveValue("1");
  await expect(page.locator("#motionResultsGrid")).toContainText("A person walks forward and waves.");
  await expect(page.locator("#motionResultsGrid video")).toHaveCount(1);
  await expect(page.locator("#motionRunBtn")).toBeDisabled();
  await page.locator("#motionScailTab").click();
