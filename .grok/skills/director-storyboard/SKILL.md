...
description: >
  ...
  storyboard (global prompt + timed local prompts + image guides), then run
  Camera Lab's Director workflow. Use when the user wants storyboard analysis,
  script-to-timeline prompting, keyframe bridging, auto-run Director, or
  runs /director-storyboard.
metadata:
